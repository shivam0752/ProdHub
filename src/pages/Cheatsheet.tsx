...
function ApmConceptCard({ c, idx }: { c: typeof APM_CORE_6[0]; idx: number }) {
  ...
  return (
    <div className="border border-white/10 rounded-[8px] overflow-hidden">
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-start gap-3 px-3 py-2.5 text-left hover:bg-white/5 transition-colors duration-fast focus:outline-none"
      >
        <span className="font-mono text-[16px] font-bold text-brand-punch opacity-60 leading-none mt-0.5 w-5 shrink-0">
          {c.rank}
        </span>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <span className="text-[12px] font-semibold text-white leading-tight">{c.concept}</span>
          </div>
          <div className="flex items-center gap-2">
            <Badge color={priorityBadge}>{c.priority}</Badge>
            <span className={`text-[10px] font-mono ${coverageColor}`}>{c.coverage}% of interviews</span>
          </div>
        </div>
        {open
          ? <ChevronUp size={13} className="text-white/30 shrink-0 mt-1" />
          : <ChevronDown size={13} className="text-white/30 shrink-0 mt-1" />
        }
      </button>

      {open && (
        <div className="px-3 pb-3 space-y-2.5 border-t border-white/10 pt-2.5">
          <p className="text-[11px] text-white/60 leading-relaxed">{c.why}</p>

          <div>
            <span className="text-[9px] font-mono uppercase tracking-widest text-brand-punch/70 block mb-1.5">Answer Structure</span>
            <ol className="space-y-1">
              {c.steps.map((s, i) => (
                <li key={i} className="text-[11px] text-white/70 flex gap-2 items-start leading-relaxed">
                  <span className="font-mono text-[9px] text-brand-punch/60 mt-0.5 shrink-0">{i + 1}.</span>
                  {s}
                </li>
              ))}
            </ol>
          ...
          <div className="bg-white/5 rounded-[6px] p-2.5">
            <span className="text-[9px] font-mono uppercase tracking-widest text-brand-positive/70 block mb-1">Example</span>
            <p className="text-[10px] text-white/60 italic leading-relaxed">{c.example}</p>
          </div>

          <div className="flex gap-1.5 items-start">
            <AlertCircle size={11} className="text-brand-danger/70 mt-0.5 shrink-0" />
            <p className="text-[10px] text-white/50 leading-relaxed">{c.mistake}</p>
          </div>

          <div className="flex flex-wrap gap-1">
            {c.triggers.map(t => (
              <span key={t} className="text-[9px] font-mono text-white/40 bg-white/5 px-2 py-0.5 rounded-none border border-white/10">
                "{t}"
              </span>
            ))}
          </div>
